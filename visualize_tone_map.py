...
<body>
    ...
    <script>
        ...
        // ── Responsive sizing ──
        // Plot area fills container width, height fits viewport
        function getPlotSize() {{
            const wrap = document.querySelector('.canvas-wrap');
            const wrapPad = 32; // 16px * 2
            const plotW = Math.max(300, wrap.clientWidth - wrapPad - margin.left - margin.right);

            const headerH = document.querySelector('.header').offsetHeight;
            const legendH = document.querySelector('.legend').offsetHeight;
            const usedH = headerH + legendH + 72; // paddings + gaps
            const plotH = Math.max(250, window.innerHeight - usedH - margin.top - margin.bottom);
        ...
        function render() {{
            ...
            canvas.width = margin.left + margin.right + plotW;
            canvas.height = margin.top + margin.bottom + plotH;
